-- ============================================================================
-- CEO DIGEST: ALL-AGENCIES AGGREGATION
-- ============================================================================
-- Extends the CEO Weekly Digest to support aggregating metrics across ALL
-- agency workspaces for a true executive-level view.
--
-- Changes:
-- 1. Add 'is_ceo_master' flag to ceo_digest_settings for org-wide digest
-- 2. Create get_ceo_digest_facts_all_agencies() RPC for aggregated metrics
-- 3. Support per-agency breakdown within the master digest
-- ============================================================================

-- ============================================================================
-- 1. ADD CEO MASTER FLAG TO SETTINGS
-- ============================================================================

ALTER TABLE ceo_digest_settings
ADD COLUMN IF NOT EXISTS is_ceo_master BOOLEAN NOT NULL DEFAULT FALSE;

COMMENT ON COLUMN ceo_digest_settings.is_ceo_master IS
  'When true, this digest aggregates metrics from ALL agency workspaces (CEO view)';

-- ============================================================================
-- 2. CREATE ALL-AGENCIES AGGREGATION RPC
-- ============================================================================
...
COMMENT ON FUNCTION get_ceo_digest_facts_all_agencies IS
  'Computes aggregated metrics across ALL agency workspaces for CEO-level digest';

-- ============================================================================
-- 3. UPDATE EXISTING SETTINGS FOR CEO MODE
-- ============================================================================

-- Mark the existing settings row as CEO master digest
UPDATE ceo_digest_settings
SET
  is_ceo_master = TRUE,
  recipients = '["blewis@lewisinsurance.com"]'::JSONB,
  enabled = TRUE,
  updated_at = NOW()
