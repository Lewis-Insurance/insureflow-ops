-- ============================================================================
-- UNIFIED CUSTOMER/INSURED/CLIENT DATA MODEL
-- ============================================================================
-- This migration unifies insureds, customers, and clients into the accounts table
-- as the single source of truth, eliminating data duplication and confusion

-- First, let's migrate any data from customers table to accounts if they don't exist
DO $$
DECLARE
...
-- Update existing search functions to use the unified approach
...
-- Replace customers_search_v1 to use unified function
...
-- Replace insureds_search_v1 to use unified function (for backward compatibility)
...
-- Create a function to get customer/insured by ID (unified)
...
AS $$
  SELECT 
    ...
    a.updated_at
  FROM public.accounts a
  WHERE a.id = customer_id AND a.deleted_at IS NULL;
$$;

-- Add RLS policies for the unified view
ALTER TABLE public.customers_unified OWNER TO postgres;

-- Enable RLS on accounts if not already enabled
ALTER TABLE public.accounts ENABLE ROW LEVEL SECURITY;

-- Add comprehensive RLS policies for accounts (customers/insureds/clients)
DROP POLICY IF EXISTS "unified_customer_access" ON public.accounts;
CREATE POLICY "unified_customer_access" 
ON public.accounts 
FOR ALL
USING (
  -- Staff can access all
  is_staff() OR 
  -- Members can access accounts they belong to
  EXISTS (
...
WITH CHECK (
  -- Staff can modify all
  is_staff() OR 
  -- Owners/staff members can modify
  EXISTS (
    SELECT 1 FROM public.account_memberships m 
...
-- Drop the separate customers table if it exists and data has been migrated
-- We'll keep it for now but mark it as deprecated
COMMENT ON TABLE public.customers IS 'DEPRECATED: Use accounts table and customers_unified view instead';
