-- =====================================================================
-- Wave 2 · DUP-1 (hardened) — merge_accounts v2 after adversarial review
-- =====================================================================
-- Fixes from the 3-lens review of the merge engine:
--  H1 Idempotency: replay guard — if every loser is already soft-deleted into
--     this survivor, return skipped_idempotent (no duplicate merge_history/flags).
--  H2 Policy safety: soft-delete within-cluster redundant policies BEFORE the
--     re-parent loop (clears the policies(policy_number) partial-unique index so
--     re-parent can't collide); and NEVER hard-delete a policy on collision (RAISE).
--  H3 Collision deletes are allowlisted to redundant derived tables only; any
--     collision on a non-allowlisted table RAISES for operator review (protects
--     client_portal_users -> portal_household_members CASCADE, insured_profiles,
--     commercial_business_accounts, household_accounts, ...).
--  H4 Reversibility: capture moved child IDs (id-PK tables) + full pre-images of
--     no-id-table children + full rows of every collision-deleted child into the
--     merge_history manifest, so an un-merge is precise.
--  H5 No-id ELSE branch gated on ACTUAL account_id uniqueness (not "lacks id").
--  H6 Field-union backfills from the cascade-best loser (policies -> completeness
--     -> updated_at -> id), not caller array order.
--  H7 Advisory lock over the SORTED full cluster; re-validate all losers active.
--  Also: compute_account_survivor completeness uses nullif(btrim()).
-- Still DRY-RUN by default. accounts only ever soft-deleted (prevent_hard_delete).
-- Date: 2026-06-28
-- =====================================================================
...
RETURNS uuid LANGUAGE sql STABLE AS $$
  ...
  LEFT JOIN LATERAL (SELECT count(*) pc FROM public.policies p WHERE p.account_id=a.id AND p.deleted_at IS NULL) pol ON true
  WHERE a.id = ANY(p_ids) AND a.deleted_at IS NULL
  ORDER BY pol.pc DESC,
    ((nullif(btrim(a.email),'') IS NOT NULL)::int
   + (nullif(btrim(a.phone),'') IS NOT NULL)::int
   + (nullif(btrim(a.address_line1),'') IS NOT NULL)::int) DESC,
    a.updated_at DESC NULLS LAST, a.id::text ASC
  LIMIT 1;
$$;
...
AS $fn$
DECLARE
  -- tables where a unique-collision on re-parent may safely DELETE the redundant
  -- loser child (derived/cache/score/membership data; full pre-image still captured).
  ...
  v_reparent       jsonb := '{}'::jsonb;   -- {table.col: count}
  v_reparent_ids   jsonb := '{}'::jsonb;   -- {table.col: [moved ids]} (id-PK tables)
  v_children_noid  jsonb := '{}'::jsonb;   -- {table.col: [full rows]} (no-id tables, pre-image)
  v_deleted_rows   jsonb := '{}'::jsonb;   -- {table.col: [full rows]} collision-deleted
...
BEGIN
  ...
  IF p_apply THEN
    -- H7: lock the whole cluster, deterministically (sorted), then row-lock.
    ...
    -- H1: idempotency / state re-validation.
  ...
  -- H6: field-union backfill from the cascade-best loser.
  ...
  -- within-cluster redundant policies (identical carrier+line+effdate+number).
  ...
  -- H2: soft-delete redundant policies BEFORE re-parent (clears partial-unique on number).
  ...
  LOOP
    ...
    -- H4: capture pre-images for reversibility.
    IF v_has_id THEN
    ...
    EXCEPTION WHEN unique_violation THEN
      ...
      IF v_has_id THEN
        ...
        END LOOP;
      ELSE
        -- H5: no-id table; only collision-resolve if account_id is actually unique AND allowlisted.
        SELECT EXISTS (
