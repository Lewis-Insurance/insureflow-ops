-- Relationship Graph v2 / Sprint 4 — merge-engine consolidation.
--
-- Before: _do_account_merge (the wrapper: assert -> edge cleanup -> same_as ->
-- consent) called the separate legacy engine merge_accounts(s, l, rule, by, apply).
-- preview_merge also called merge_accounts(..., apply=false) for its dry run.
--
-- After: _do_account_merge IS the single merge body. It takes an `apply boolean`
-- (default true): apply=false is a pure compute path (no mutation) that returns
-- the same counts/field_union preview needs; apply=true does the full merge plus
-- the same_as provenance edge and consent reconciliation. preview_merge is
-- repointed at _do_account_merge(..., apply=false), and the legacy 5-arg
-- merge_accounts is dropped. The policy de-dup key now normalizes & <-> and on the
-- carrier so "AT&T" and "AT and T" collapse.
--
-- _do_account_merge stays EXECUTE service_role/postgres only (the gated wrappers
-- merge_accounts_manual / relgraph_merge_duplicate_group / preview_merge reach it
-- as SECURITY DEFINER, so the grant restriction only blocks direct external calls).
--
-- Rollback: recreate merge_accounts (migration 20260629180000) and the prior
-- 3-arg _do_account_merge + preview_merge bodies (migration 20260629160000).

-- 3-arg signature must go before the 4-arg can be created; callers late-bind by name.
drop function if exists public._do_account_merge(uuid, uuid[], text);

create function public._do_account_merge(p_survivor uuid, p_losers uuid[], p_rule text, p_apply boolean default true)
 returns jsonb
 language plpgsql
 security definer
...
begin
  ...
  -- mutating pre-steps only on apply
  if p_apply then
  ...
  if p_apply then
    ...
    -- provenance + consent (the former _do_account_merge wrapper, now inline)
...
revoke execute on function public._do_account_merge(uuid, uuid[], text, boolean) from public, anon, authenticated;
grant  execute on function public._do_account_merge(uuid, uuid[], text, boolean) to service_role;

-- preview_merge: repoint its dry-run engine at _do_account_merge(..., apply=false)
...
begin
  ...
  v_counts := public._do_account_merge(p_survivor, v_active, 'preview', false);
...
-- legacy engine retired: nothing references it anymore
drop function if exists public.merge_accounts(uuid, uuid[], text, uuid, boolean);
