-- Profile Hardening Pack Database Schema (Only new tables and missing columns)
...
CREATE INDEX IF NOT EXISTS idx_email_change_requests_user_status ON email_change_requests(user_id, status);
CREATE INDEX IF NOT EXISTS idx_role_change_requests_user_status ON role_change_requests(user_id, status);
CREATE INDEX IF NOT EXISTS idx_user_sessions_user_active ON user_sessions(user_id, last_active DESC);
CREATE INDEX IF NOT EXISTS idx_profile_access_logs_target_created ON profile_access_logs(target_user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_data_export_requests_user_status ON data_export_requests(user_id, status);
CREATE INDEX IF NOT EXISTS idx_phone_verification_user_expires ON phone_verification_codes(user_id, expires_at);
CREATE INDEX IF NOT EXISTS idx_impersonation_logs_target_started ON impersonation_logs(target_user_id, started_at DESC);
