...
-- Now create the unified data model

-- Create a unified view that presents accounts as "customers" for backward compatibility
DROP VIEW IF EXISTS public.customers_unified;
CREATE VIEW public.customers_unified AS
SELECT 
  a.id,
  a.id as account_id,
  a.name,
...
-- Recreate customers_search to use the unified approach
...
-- Recreate customers_search_v1 to use unified function
...
-- Recreate insureds_search_v1 to use unified function (for backward compatibility)
