      ...
      WITH paired AS (
        SELECT 
          ...
          GREATEST(
            ...
            COALESCE(extensions.similarity(a.name, b.name), 0)
          ) as score
        FROM accounts a
      ...
      WITH paired AS (
        SELECT 
          ...
          GREATEST(
            CASE WHEN a.email IS NOT NULL AND a.email = b.email THEN 1.0 ELSE 0 END,
            CASE WHEN a.phone IS NOT NULL AND a.phone = b.phone THEN 0.95 ELSE 0 END,
            COALESCE(extensions.similarity(a.first_name || ' ' || a.last_name, b.first_name || ' ' || b.last_name), 0)
          ) as score
        FROM contacts a
...
-- Profile access logs - users can see their own access logs, staff can see all
...
-- Task management - staff only
CREATE POLICY "staff_only_tasks"
ON public.tasks FOR ALL
USING (public.is_staff())
WITH CHECK (public.is_staff());

-- Events - staff only  
CREATE POLICY "staff_only_events"
...
-- 8. CREATE AUDIT TRIGGER FOR SENSITIVE OPERATIONS
...
CREATE TRIGGER audit_policies_access  
  AFTER INSERT OR UPDATE OR DELETE ON public.policies
  FOR EACH ROW EXECUTE FUNCTION public.log_sensitive_access();

CREATE TRIGGER audit_claims_access
