-- ============================================================================
-- FIX: CEO DIGEST TOP_AGENTS - policies.created_by doesn't exist
-- ============================================================================
-- The policies table doesn't have a created_by column.
-- The quotes table DOES have created_by.
-- Fix: Use quotes won (status='won') to measure agent performance instead.
-- ============================================================================
...
BEGIN
  ...
  BEGIN
    SELECT COALESCE(COUNT(*), 0) INTO v_prev_leads_new
    FROM leads l WHERE l.created_at >= v_prev_period_start AND l.created_at < v_prev_period_end AND l.deleted_at IS NULL;
  EXCEPTION WHEN undefined_table OR undefined_column THEN NULL;
  END;

  BEGIN
    SELECT COALESCE(COUNT(*), 0) INTO v_prev_quotes_created
    FROM quotes q WHERE q.created_at >= v_prev_period_start AND q.created_at < v_prev_period_end;
  EXCEPTION WHEN undefined_table OR undefined_column THEN NULL;
  END;
  ...
  BEGIN
    ...
    FROM policies p
    WHERE p.created_at >= v_prev_period_start AND p.created_at < v_prev_period_end
  ...
  BEGIN
    ...
    FROM canopy_pulls cp WHERE cp.created_at >= p_period_start AND cp.created_at < p_period_end;
  ...
  BEGIN
    SELECT COALESCE(jsonb_agg(agency_data ORDER BY premium_written DESC), '[]'::JSONB) INTO v_by_agency
    FROM (
      SELECT jsonb_build_object(
        ...
        'tasks_overdue', COALESCE((SELECT COUNT(*) FROM tasks t WHERE t.agency_workspace_id = aw.id AND t.status = 'pending' AND t.due_at < NOW()), 0)
      ) AS agency_data,
  ...
  v_deltas := jsonb_build_object(
    'leads_new', jsonb_build_object(
      'current', v_leads_new, 'previous', v_prev_leads_new,
      'change', v_leads_new - v_prev_leads_new,
      'change_pct', CASE WHEN v_prev_leads_new > 0 THEN ROUND(((v_leads_new::NUMERIC - v_prev_leads_new) / v_prev_leads_new * 100)::NUMERIC, 1) ELSE NULL END
    ),
    'quotes_created', jsonb_build_object(
      'current', v_quotes_created, 'previous', v_prev_quotes_created,
      'change', v_quotes_created - v_prev_quotes_created,
      'change_pct', CASE WHEN v_prev_quotes_created > 0 THEN ROUND(((v_quotes_created::NUMERIC - v_prev_quotes_created) / v_prev_quotes_created * 100)::NUMERIC, 1) ELSE NULL END
    ),
    'policies_bound', jsonb_build_object(
      'current', v_policies_bound, 'previous', v_prev_policies_bound,
      'change', v_policies_bound - v_prev_policies_bound,
      'change_pct', CASE WHEN v_prev_policies_bound > 0 THEN ROUND(((v_policies_bound::NUMERIC - v_prev_policies_bound) / v_prev_policies_bound * 100)::NUMERIC, 1) ELSE NULL END
    ),
    'premium_written', jsonb_build_object(
      'current', v_premium_written, 'previous', v_prev_premium_written,
      'change', v_premium_written - v_prev_premium_written,
      'change_pct', CASE WHEN v_prev_premium_written > 0 THEN ROUND(((v_premium_written - v_prev_premium_written) / v_prev_premium_written * 100)::NUMERIC, 1) ELSE NULL END
    )
  );

  -- Build funnel
  v_funnel := jsonb_build_object(
    'leads', jsonb_build_object('new', v_leads_new, 'contacted', v_leads_contacted, 'qualified', v_leads_qualified, 'quoted', v_leads_quoted, 'won', v_leads_won, 'lost', v_leads_lost),
    'quotes', jsonb_build_object('created', v_quotes_created, 'open', v_quotes_open, 'won', v_quotes_won, 'lost', v_quotes_lost)
  );

  -- Build lists
  -- FIXED: top_agents now uses quotes.created_by instead of policies.created_by
  v_lists := jsonb_build_object(
    'top_opportunities', (
      SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB)
      FROM (
        SELECT q.id,
          CASE WHEN p_include_pii THEN a.name ELSE LEFT(a.name, 1) || '***' END AS account_name,
          q.line_of_business, q.premium, q.status::TEXT, '/quotes/' || q.id AS deep_link
        FROM quotes q JOIN accounts a ON q.account_id = a.id
        WHERE q.status = 'open' AND q.premium > 0
        ORDER BY q.premium DESC LIMIT 10
      ) t
    ),
    'aging_quotes', (
      SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB)
      FROM (
        SELECT q.id,
          CASE WHEN p_include_pii THEN a.name ELSE LEFT(a.name, 1) || '***' END AS account_name,
          q.line_of_business, q.premium,
          EXTRACT(DAY FROM NOW() - q.created_at)::INTEGER AS days_old,
          '/quotes/' || q.id AS deep_link
        FROM quotes q JOIN accounts a ON q.account_id = a.id
        WHERE q.status = 'open' AND q.created_at < NOW() - (v_aging_quotes_days || ' days')::INTERVAL
        ORDER BY q.created_at ASC LIMIT 10
      ) t
    ),
    'top_agents', (
      -- FIXED: Using quotes.created_by (quotes won) instead of policies.created_by
      SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB)
      FROM (
        SELECT
          p.id AS agent_id,
          CASE WHEN p_include_pii THEN p.full_name
               ELSE SPLIT_PART(p.full_name, ' ', 1) || ' ' || LEFT(COALESCE(SPLIT_PART(p.full_name, ' ', 2), ''), 1) || '.' END AS agent_name,
          aw.name AS agency_name,
          COUNT(q.id) AS quotes_won_this_week,
          COALESCE(SUM(q.premium), 0)::NUMERIC AS premium_quoted
        FROM profiles p
        JOIN agency_workspace_memberships m ON m.user_id = p.id
        JOIN agency_workspaces aw ON aw.id = m.agency_workspace_id
        LEFT JOIN quotes q ON q.created_by = p.id
          AND q.status = 'won'
          AND q.updated_at >= p_period_start
          AND q.updated_at < p_period_end
        WHERE m.role IN ('producer', 'admin', 'owner')
          AND m.status = 'active'
        GROUP BY p.id, p.full_name, aw.name
        ORDER BY premium_quoted DESC
        LIMIT 10
      ) t
    ),
    'top_carriers', (
      ...
      FROM (
        ...
        WHERE pol.created_at >= p_period_start AND pol.created_at < p_period_end AND pol.deleted_at IS NULL
        GROUP BY c.id, c.name
        ORDER BY premium_written DESC LIMIT 10
      ) t
    ),
    'top_lines_of_business', (
      SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB)
      FROM (
        SELECT pol.line_of_business, COUNT(pol.id) AS policies_this_week,
          COALESCE(SUM(pol.premium), 0)::NUMERIC AS premium_written
        FROM policies pol
        WHERE pol.created_at >= p_period_start AND pol.created_at < p_period_end
          AND pol.deleted_at IS NULL AND pol.line_of_business IS NOT NULL
  ...
  v_service_ops := jsonb_build_object(
    ...
    'tasks_by_priority', (SELECT COALESCE(jsonb_object_agg(priority, cnt), '{}'::JSONB) FROM (SELECT priority, COUNT(*) AS cnt FROM tasks WHERE status = 'pending' GROUP BY priority) t),
    'tasks_by_category', (SELECT COALESCE(jsonb_object_agg(entity_type, cnt), '{}'::JSONB) FROM (SELECT COALESCE(entity_type, 'other') AS entity_type, COUNT(*) AS cnt FROM tasks WHERE status = 'pending' GROUP BY entity_type) t),
    'overdue_tasks_list', (
      SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB)
      FROM (
  ...
  v_integration_health := jsonb_build_object(
    'canopy', CASE WHEN v_canopy_pulls IS NOT NULL THEN jsonb_build_object(
      ...
      'error_rate_pct', CASE WHEN v_canopy_pulls > 0 THEN ROUND((v_canopy_errors::NUMERIC / v_canopy_pulls * 100)::NUMERIC, 1) ELSE 0 END
    ) ELSE jsonb_build_object('available', FALSE, 'missing', TRUE) END
  );

  -- Generate alerts
  IF v_prev_leads_new > 0 AND ((v_prev_leads_new - v_leads_new)::NUMERIC / v_prev_leads_new * 100) >= v_leads_drop_pct THEN
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object(
      'severity', 'warning', 'category', 'leads', 'title', 'Significant drop in new leads (org-wide)',
      'message', 'New leads dropped by ' || ROUND(((v_prev_leads_new - v_leads_new)::NUMERIC / v_prev_leads_new * 100)::NUMERIC, 1) || '% compared to last week (' || v_leads_new || ' vs ' || v_prev_leads_new || ')',
      'evidence', jsonb_build_object('current', v_leads_new, 'previous', v_prev_leads_new)
    ));
  END IF;

  IF v_prev_quotes_created > 0 AND ((v_prev_quotes_created - v_quotes_created)::NUMERIC / v_prev_quotes_created * 100) >= v_quotes_drop_pct THEN
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object(
      'severity', 'warning', 'category', 'quotes', 'title', 'Significant drop in quotes (org-wide)',
      'message', 'Quotes created dropped by ' || ROUND(((v_prev_quotes_created - v_quotes_created)::NUMERIC / v_prev_quotes_created * 100)::NUMERIC, 1) || '% compared to last week (' || v_quotes_created || ' vs ' || v_prev_quotes_created || ')',
  ...
  IF v_tasks_overdue >= v_overdue_tasks_critical THEN
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object(
      'severity', 'critical', 'category', 'operations', 'title', 'Critical backlog of overdue tasks (org-wide)',
  ...
  IF v_canopy_reconnects IS NOT NULL AND v_canopy_reconnects >= v_canopy_reconnects_critical THEN
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object(
      'severity', 'critical', 'category', 'integration', 'title', 'Canopy connections need attention',
  ...
  IF v_canopy_errors IS NOT NULL AND v_canopy_errors >= v_canopy_errors_critical THEN
    v_alerts := v_alerts || jsonb_build_array(jsonb_build_object(
      'severity', 'warning', 'category', 'integration', 'title', 'Elevated Canopy sync errors',
...
COMMENT ON FUNCTION get_ceo_digest_facts_all_agencies IS
  'CEO digest facts aggregated across all agencies. Fixed: top_agents uses quotes.created_by instead of policies.created_by';
