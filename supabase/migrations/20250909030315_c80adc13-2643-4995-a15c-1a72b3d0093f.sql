...
-- This provides backward compatibility while using the new canonical search
...
AS $$
BEGIN
  -- Delegate to the canonical customers search function
  -- Convert the old cursor-based pagination to offset-based for compatibility
  RETURN QUERY
  SELECT 
    c.account_id,
    c.display_name,
  ...
  FROM public.customers_search_v1(
    p_filters,
    p_limit,
    0, -- offset = 0 for now (cursor pagination conversion is complex)
    p_sort
  ) c;
END;
$$;
