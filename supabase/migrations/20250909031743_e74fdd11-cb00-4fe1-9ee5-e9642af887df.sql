-- Drop the existing profiles table if it exists and create as a view instead
-- First check if there's data we need to preserve
DO $$ 
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'profiles') THEN
        -- If profiles table exists and has data beyond auth.users references, this might need manual migration
        RAISE NOTICE 'profiles table exists - checking if safe to drop';
        
        -- For this migration, we'll assume it's safe to drop and recreate as view
        -- In production, you'd want to backup any custom data first
