-- ============================================================================
-- Phase 4 Workers Comp: line-aware quote/bind extension + class-code seed
-- ============================================================================
-- Adds 'wc' to the atomic submission RPCs. Gotchas handled explicitly:
--   - quotes.line_of_business is the enum line_of_business whose WC label is
--     'workers_comp' (NOT 'wc'): the RPC maps the module's line key to the
--     enum label on insert, and the bind guard normalizes both sides.
--   - Old signatures are DROPPED first (overloads break PostgREST rpc
--     resolution); new params default null so gl/property callers are
--     unchanged.
-- WC bind requires ALL THREE employers-liability limits - they are the
-- property line's required_for_ready trio on the COI registry (verified
-- live): wc_details.coverage.part_two_employers_liability.{each_accident,
-- disease_each_employee, disease_policy_limit}.
-- Also seeds ~30 common FL workers-comp class codes into the wc_class_codes
-- reference table (Phase 0 schema) for the class picker; ON CONFLICT DO
-- NOTHING keeps re-runs and local additions safe.

drop function if exists public.add_submission_quote(uuid, text, numeric, numeric, numeric, text, numeric);
...
comment on function public.add_submission_quote(uuid, text, numeric, numeric, numeric, text, numeric, numeric, numeric, numeric) is
  'Atomic quote capture on a commercial submission, line-aware (gl | property | wc). Staff + workspace gated inside; SECURITY DEFINER.';
revoke execute on function public.add_submission_quote(uuid, text, numeric, numeric, numeric, text, numeric, numeric, numeric, numeric) from anon, public;
grant  execute on function public.add_submission_quote(uuid, text, numeric, numeric, numeric, text, numeric, numeric, numeric, numeric) to authenticated;

drop function if exists public.bind_submission_quote(uuid, uuid, numeric, numeric, text, numeric, text);
...
comment on function public.bind_submission_quote(uuid, uuid, numeric, numeric, text, numeric, text, numeric, numeric, numeric) is
  'Atomic line-aware bind (gl | property | wc): line derived from the quote, line-required limits enforced (WC = all three EL limits), save_master_coi_fields rejections fail the bind, won/lost + event + bound in one transaction under a submission row lock. Staff + workspace gated; SECURITY DEFINER.';
revoke execute on function public.bind_submission_quote(uuid, uuid, numeric, numeric, text, numeric, text, numeric, numeric, numeric) from anon, public;
grant  execute on function public.bind_submission_quote(uuid, uuid, numeric, numeric, text, numeric, text, numeric, numeric, numeric) to authenticated;

-- ---------------------------------------------------------------------------
-- Common FL workers-comp class codes (reference picker seed; descriptions are
-- generic public vocabulary, not NCCI text). Idempotent.
-- ---------------------------------------------------------------------------
insert into public.wc_class_codes (code, state, description) values
  ('0042','FL','Landscaping and gardening'),
  ('2003','FL','Bakery and delivery'),
  ('3632','FL','Machine shop'),
  ('5183','FL','Plumbing'),
  ('5190','FL','Electrical wiring'),
  ('5221','FL','Concrete or cement work'),
  ('5403','FL','Carpentry NOC'),
  ('5437','FL','Carpentry, interior trim'),
  ('5445','FL','Wallboard installation'),
  ('5474','FL','Painting NOC'),
  ('5478','FL','Flooring installation'),
  ('5537','FL','HVAC installation and service'),
  ('5551','FL','Roofing'),
  ('5606','FL','Contractor, executive supervisor'),
  ('5645','FL','Carpentry, residential'),
  ('7219','FL','Trucking NOC'),
  ('8001','FL','Florist store'),
  ('8006','FL','Grocery store'),
  ('8010','FL','Hardware store'),
  ('8017','FL','Retail store NOC'),
  ('8380','FL','Automobile repair shop'),
  ('8742','FL','Salespersons, outside'),
  ('8810','FL','Clerical office employees'),
  ('8820','FL','Attorneys, all employees'),
  ('8832','FL','Physicians and clerical'),
  ('8835','FL','Home health care'),
  ('9014','FL','Janitorial services'),
  ('9052','FL','Hotels, all other employees'),
  ('9082','FL','Restaurant NOC'),
  ('9102','FL','Lawn maintenance')
on conflict (code, state) do nothing;
