...
//
// Two modes (opts.mode):
//   'policy' (default): both rules are errors. This is the original behavior;
//     omitting opts changes nothing.
//   'packet': the submission-application gate. Submissions are created
//     lines-first (the target effective date is optional at intake), so a
//     legitimately-sparse fresh application would always trip R2; it
//     downgrades to a warning. R1 stays an error in every mode: an
//     application with no applicant is meaningless.
//
...
export interface ValidateAcord125Options {
  /**
   * 'policy' (default) hard-requires the R2 effective date; 'packet'
   * downgrades it to a warning (see header comment).
   */
  mode?: 'policy' | 'packet';
}
...
export function validateAcord125(
  input: Acord125Input,
  opts: ValidateAcord125Options = {},
): ValidateAcord125Result {
  const mode = opts.mode ?? 'policy';
  const issues: Acord125Issue[] = [];

  // R1: named insured name (an error in every mode).
  ...
  if (anyLineChecked && effectiveDate.length === 0) {
    issues.push({
      ...
      severity: mode === 'packet' ? 'warning' : 'error',
      message:
