...
async function processCanopyPolicy(supabase: ReturnType<typeof createClient>, pullId: string, policy: any) {
  ...
  // Process driving records (may be at policy or driver level)
  for (const record of policy.driving_records || []) {
    // Need to find the associated driver ID
    if (record.driver_id) {
...
async function upsertDrivingRecord(
  ...
  // Parse amount from cents if needed
...
async function upsertLossEvent(
  ...
  const lossAmount = event.loss_amount_cents
    ? event.loss_amount_cents / 100
    : event.loss_amount;
  const paidAmount = event.paid_amount_cents
    ? event.paid_amount_cents / 100
    : event.paid_amount || event.amount_paid;
  const reservedAmount = event.reserved_amount_cents
    ? event.reserved_amount_cents / 100
    : event.reserved_amount || event.amount_reserved;
  const deductibleAmount = event.deductible_cents
    ? event.deductible_cents / 100
    : event.deductible || event.deductible_applied;
...
async function upsertAddress(
  ...
  // Try to find by Canopy ID first
  if (canopyAddressId) {
    const { data: existing } = await supabase
  ...
  // Fallback: find by address components
...
// ============================================================================
// ENHANCED VEHICLE/DWELLING UPSERTS WITH NEW FIELDS
// ============================================================================
...
async function upsertDwelling(supabase: ReturnType<typeof createClient>, policyId: string, dwelling: any) {
  ...
  const dwellingData = {
    ...
    property_type: dwelling.property_type || propData.property_type,
    occupancy_type: dwelling.occupancy_type || propData.occupancy_type,
...
async function refreshSpecificPull(
  ...
  try {
    ...
    // =========================================================================
    // PROCESS PULL-LEVEL DATA (addresses, loss_events, driving_records)
    // =========================================================================

    // Process addresses from pull level
    const pullAddresses = pullData.addresses || [];
    console.log(`[Canopy Reprocess] Pull-level addresses: ${pullAddresses.length}`);
    for (const addr of pullAddresses) {
      await upsertAddress(supabase, internalPullId, addr, addr.address_type || 'mailing');
    }

    // Process loss events from pull level
    const pullLossEvents = pullData.loss_events || pullData.loss_history || [];
    console.log(`[Canopy Reprocess] Pull-level loss events: ${pullLossEvents.length}`);
    for (const event of pullLossEvents) {
      await upsertLossEvent(supabase, internalPullId, event);
    }

    // Process pull-level driving records
    const pullDrivingRecords = pullData.driving_records || [];
    console.log(`[Canopy Reprocess] Pull-level driving records: ${pullDrivingRecords.length}`);
    if (pullDrivingRecords.length > 0) {
      // Try to associate with a driver - get the first driver from the pull
      const { data: firstDriver } = await supabase
        .from('canopy_drivers')
        .select('id, canopy_policies!inner(pull_id)')
        .eq('canopy_policies.pull_id', internalPullId)
        .limit(1)
        .single();

      if (firstDriver) {
        for (const record of pullDrivingRecords) {
          await upsertDrivingRecord(supabase, firstDriver.id, record);
        }
      }
    }
