...
serve(async (req) => {
  ...
  try {
    ...
    for (const account of accounts) {
      ...
      // Store in database
      const { data: riskScoreData } = await supabaseClient.from("customer_risk_scores").upsert({
      ...
      // Auto-generate retention tasks for medium/high/critical risk customers
      if (riskScoreData && prediction.churnRiskLevel !== 'low' && prediction.recommendedActions.length > 0) {
        try {
          // Check if tasks already exist for this account to avoid duplicates
          ...
          // Only create tasks if no recent retention tasks exist
          if (!existingTasks || existingTasks.length === 0) {
            const tasksToCreate = [];

            // Generate tasks from top 3 recommended actions
            for (const action of prediction.recommendedActions.slice(0, 3)) {
              const dueDate = new Date();
              dueDate.setDate(dueDate.getDate() + action.due_days);

              tasksToCreate.push({
                account_id: account.id,
                title: `${action.action} - ${account.name}`,
                description: `${action.rationale}

**Churn Risk:** ${prediction.churnProbability}% (${prediction.churnRiskLevel.toUpperCase()})
**Days Until Renewal:** ${prediction.daysUntilRenewal !== null ? prediction.daysUntilRenewal : 'Unknown'}
...
This task was automatically generated by the Predictive Analytics Engine based on customer risk factors.`,
            ...
            if (tasksToCreate.length > 0) {
              await supabaseClient.from('tasks').insert(tasksToCreate);
              console.log(`✓ Auto-generated ${tasksToCreate.length} retention task(s) for ${account.name}`);
            }
          }
        } catch (taskError) {
          // Log error but don't fail the entire risk calculation
          console.error(`Failed to generate tasks for ${account.name}:`, taskError);
        }
      }
