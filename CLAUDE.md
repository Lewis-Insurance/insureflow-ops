...
- ✅ **Renewals hardening** (`20260702100000` + `20260702101500`): sync trigger/reconciler skip soft-deleted policies and follow the open renewal per policy (endorsement date edits move the row, no duplicates); new atomic `renewal_mark_renewed`/`renewal_mark_lost` RPCs (close renewal FIRST, then policy - hooks rewired); `renewal_reopen` reordered + pristine-sibling cleanup; `renewal_mark_moved` dup guard scoped to live policies; **partial unique index `renewals_one_open_per_policy`** over the full open-status set (pending/contacted/quoted/upcoming/in_progress - DB predicates now match the frontend's OPEN_STATUSES); draft autosave no-ops if the renewal closed mid-flight; local-date parsing fixes (off-by-one day). End-to-end flow verified against prod in a rolled-back transaction. NOTE: prod functions had drifted from repo migration files (hand patches); the new migrations were generated FROM live prod definitions.
- ✅ **Merge/customers**: `invalidateAccountDataCaches()` after every merge/unmerge (survivor no longer looks unmerged for 5 min); duplicate queue paginated (was hard-capped at 200); merge search comma-safe + debounced; `usePolicies` gains `accountId` server-side filter + excludes soft-deleted rows (customer record no longer paginates the whole book on open); duplicate-policy dialog matches the active-only unique index (no more wrong-customer Merge CTA); survivor picker excludes tombstoned members; Link-instead links all members of 3+ groups.
- ✅ **Payments/day-sheets**: one-shot auto-print (after payments load, URL param cleared); edit preserves check/reference numbers when the method list is unavailable; Add Payment on a sheet defaults to that sheet's date; `useCurrentDaySheet` ET-pinned + date-keyed; voided+NSF excluded from all collected totals; amount rounded to cents; `usePayments` method-type filter fixed (embedded `.in()` never filtered) + search sanitized. `sanitizeMultiFieldSearch` now emits PostgREST-quoted patterns (commas no longer 400).
- ✅ **Notes/customer record**: per-policy note/task/doc modals carry `policy_id`; Log-contact invalidates communication-history (was invisible → duplicate logging); NotesPanel real error state (E&O trail must not fake-empty); dead notes state/fetches removed from CustomerDetail; task tiles share the section's task list.
- ✅ **Theme**: cc-* palette wrapped in `color-mix` so `/opacity` modifiers emit CSS (14 broken call sites fixed with zero call-site edits); pre-paint theme script in index.html (no dark flash for light users); unused Plus Jakarta Sans dropped; extraction-review badge + prism label contrast; 16 missing tokens synced into `design-system/design-tokens.css`.
- ✅ **Search races**: monotonic request guards in `useGlobalSearch`/`useAccountSearch`/`useLeadSearch`.
- ✅ **Importer**: bounded concurrency (8 in flight, was 1 RPC per row sequential), 25-consecutive-failure circuit breaker, `completed_with_errors` batch status (dead ternary fixed), matched accounts counted in successful_rows, CSV parser handles quoted embedded newlines. Deleted dead/dangerous code: legacy `CSVImport.tsx`, `RenewalNotes.tsx` + `useRenewalNotes` family (hard-deletes), `components/renewals/BulkActionsBar.tsx`, `integrations/supabase/hooks/useRenewals.ts`.
- **Deferred (explicitly, from the review's P2 list)**: `unmerge_account` to_jsonb row-matching redesign; merge field-union extension beyond 11 fields (loser data retained in `losers_before` snapshots); fresh-DB migration replay validation (prod was hand-applied; replay untested); `get_account_cluster` per-row subquery; renewals whole-book fetch virtualization; ILIKE `%` wildcard leakage in SQL RPCs (parameterized + limit-capped, not injectable); EditRelationshipDrawer owner-direction control; broader error boundaries; PaymentList "Today" frozen-at-mount edge.
