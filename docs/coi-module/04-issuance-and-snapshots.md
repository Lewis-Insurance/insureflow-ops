...
- `template_id`/`template_version`/`acord_edition` are typed columns for querying ("every cert issued on the 2016/03 edition") even though the same values are frozen inside `snapshot`. This implements the handoff's edition-drift rule (docs/archive/ACORD_COI_Module_Handoff.md Section 5, form edition drift).
