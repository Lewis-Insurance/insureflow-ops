...
CREATE INDEX idx_portal_branding_code ON portal_branding(agency_code);
...
CREATE INDEX idx_portal_users_auth ON client_portal_users(auth_user_id);
CREATE INDEX idx_portal_users_account ON client_portal_users(account_id);
CREATE INDEX idx_portal_users_email ON client_portal_users(email);
CREATE INDEX idx_portal_users_status ON client_portal_users(portal_status);
CREATE INDEX idx_portal_users_branding ON client_portal_users(branding_id);
...
CREATE INDEX idx_household_primary ON portal_household_members(primary_user_id);
CREATE INDEX idx_household_auth ON portal_household_members(auth_user_id);
CREATE INDEX idx_household_status ON portal_household_members(status);
...
CREATE INDEX idx_provenance_policy ON policy_data_provenance(policy_id);
CREATE INDEX idx_provenance_field ON policy_data_provenance(policy_id, field_name);
CREATE INDEX idx_provenance_current ON policy_data_provenance(policy_id, is_current) WHERE is_current = TRUE;

-- CRITICAL: Enforce only one current value per field per policy
CREATE UNIQUE INDEX uniq_provenance_current_field 
ON policy_data_provenance(policy_id, field_name) 
WHERE is_current = TRUE;

-- ----------------------------------------------------------------------------
-- 1.5 PORTAL DOCUMENTS
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS portal_documents (
  ...
  uploaded_by_portal_user_id UUID REFERENCES client_portal_users(id),
  ...
  -- Download tracking (proper FKs)
  download_count INTEGER DEFAULT 0,
  last_downloaded_at TIMESTAMPTZ,
  last_downloaded_by_portal_user_id UUID REFERENCES client_portal_users(id),
  last_downloaded_by_household_member_id UUID REFERENCES portal_household_members(id),
...
CREATE INDEX idx_portal_docs_account ON portal_documents(account_id);
CREATE INDEX idx_portal_docs_policy ON portal_documents(policy_id);
CREATE INDEX idx_portal_docs_type ON portal_documents(document_type);
CREATE INDEX idx_portal_docs_visible ON portal_documents(account_id, is_client_visible, verified_for_client_view) 
...
CREATE INDEX idx_id_cards_account ON portal_id_cards(account_id);
CREATE INDEX idx_id_cards_policy ON portal_id_cards(policy_id);
CREATE INDEX idx_id_cards_active ON portal_id_cards(account_id, is_active) WHERE is_active = TRUE;
...
CREATE INDEX idx_service_requests_account ON portal_service_requests(account_id);
CREATE INDEX idx_service_requests_user ON portal_service_requests(portal_user_id);
CREATE INDEX idx_service_requests_status ON portal_service_requests(status);
CREATE INDEX idx_service_requests_assigned ON portal_service_requests(assigned_to);
CREATE INDEX idx_service_requests_sla ON portal_service_requests(sla_due_at) WHERE status NOT IN ('completed', 'cancelled');
...
CREATE INDEX idx_request_messages_request ON portal_service_request_messages(request_id);
CREATE INDEX idx_request_messages_visible ON portal_service_request_messages(request_id, is_internal) WHERE is_internal = FALSE;
...
CREATE INDEX idx_doc_uploads_account ON portal_document_uploads(account_id);
CREATE INDEX idx_doc_uploads_user ON portal_document_uploads(portal_user_id);
CREATE INDEX idx_doc_uploads_extraction ON portal_document_uploads(extraction_status);
CREATE INDEX idx_doc_uploads_agent_verification ON portal_document_uploads(agent_verification_status) WHERE agent_verification_status = 'pending';
...
CREATE INDEX idx_quote_requests_account ON portal_quote_requests(account_id);
CREATE INDEX idx_quote_requests_user ON portal_quote_requests(portal_user_id);
CREATE INDEX idx_quote_requests_status ON portal_quote_requests(status);
...
CREATE INDEX idx_referrals_user ON portal_referrals(referring_user_id);
CREATE INDEX idx_referrals_account ON portal_referrals(referring_account_id);
CREATE INDEX idx_referrals_status ON portal_referrals(status);
CREATE INDEX idx_referrals_code ON portal_referrals(referral_code);
...
CREATE INDEX idx_opportunities_user ON portal_coverage_opportunities(portal_user_id);
CREATE INDEX idx_opportunities_account ON portal_coverage_opportunities(account_id);
CREATE INDEX idx_opportunities_active ON portal_coverage_opportunities(account_id, status) WHERE status = 'active';
...
CREATE INDEX idx_activity_user ON portal_activity_log(portal_user_id);
CREATE INDEX idx_activity_household ON portal_activity_log(household_member_id);
CREATE INDEX idx_activity_type ON portal_activity_log(activity_type);
CREATE INDEX idx_activity_date ON portal_activity_log(created_at);
CREATE INDEX idx_activity_automation ON portal_activity_log(processed_for_automation) WHERE processed_for_automation = FALSE;
...
CREATE INDEX idx_invitations_account ON portal_invitations(account_id);
CREATE INDEX idx_invitations_email ON portal_invitations(email);
CREATE INDEX idx_invitations_status ON portal_invitations(status);
...
CREATE INDEX idx_emergency_user ON portal_emergency_log(portal_user_id);
CREATE INDEX idx_emergency_expires ON portal_emergency_log(expires_at);
...
-- ============================================================================
-- SECTION 7: STORAGE NOTES (IMPORTANT!)
-- ============================================================================
/*
STORAGE STRATEGY (Implement in Supabase Dashboard or via API):

1. Create a PRIVATE bucket for portal documents:
   - Name: portal-documents
   - Public: FALSE
   - File size limit: 50MB
   - Allowed mime types: application/pdf, image/*, application/msword, etc.

2. All file access MUST go through Edge Functions that:
   a) Verify user has access via account_id check
   b) Issue short-lived signed URLs (5-15 minutes)
   c) Log the access to portal_activity_log

3. NEVER expose direct storage URLs to clients

4. Example Edge Function pattern:
   ...
   export async function handler(req) {
     ...
     const { data: doc } = await supabase
       ...
       .select('file_path, account_id')
       .eq('id', documentId)
       .single();
     
     const { data: access } = await supabase
       .rpc('portal_accessible_account_ids')
     ...
     await supabase.rpc('log_my_portal_activity', {
       p_activity_type: 'download_document',
       p_activity_data: { document_id: documentId }
     });
