...
**V1 = coi.issue, reactive, on existing rails.** Build order: (1) close R7 = `stage_client_send()` chokepoint + route `send-coi-email` through it + finish the cert-access TODO; (2) resolve-account; (3) gate allowlist on SPF/DKIM/DMARC; (4) redactPII before extract, fill ACORD 25 by reference; (5) router shim (`classify-document` 'coi' from allowlisted auth-passing sender → `action='coi.issue'`); (6) the coi.issue play (resolve → policy snapshot → fill → coverage diff → park `awaiting_approval`); (7) render the Slack card, sub-5s ack, held undo, log FeedbackEvent. Scope tight: repeat holders whose demand the diff verifies vs an active policy; everything else falls through unchanged. Adoption hinge is **Landen** (runs certs), tying to the Phase 1 DoD; Kelli's clout lands later on the red-flag remarket.
