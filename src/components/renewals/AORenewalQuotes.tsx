    ...
    <>
      <div className="space-y-4">
        ...
        <div className="rounded-3xl border border-white/10 bg-[#0f1728]/80 p-4">
          ...
          {isLoading ? (
            <div className="py-8 text-center text-slate-400">Loading quotes...</div>
          ) : quotes.length === 0 ? (
                        ...
                        <TableRow key={quote.id} className="border-white/10 hover:bg-white/5">
                                ...
                                <div className="flex items-center gap-1.5">
                                  {savings.amount > 0 ? (
                                    <>
                                      ...
                                      <span className="font-medium text-emerald-300">Save {formatCurrency(savings.amount)}</span>
                                    </>
                                  ) : (
                                    <>
                                      <TrendingUp className="h-4 w-4 text-rose-300" />
                                      <span className="font-medium text-rose-300">+{formatCurrency(Math.abs(savings.amount))}</span>
                                    </>
                                  )}
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              <Button variant="ghost" size="sm" className="text-slate-300 hover:bg-white/10 hover:text-white" onClick={() => setEditQuote(quote)}>
                                <Pencil className="h-4 w-4" />
                              </Button>
                              <Button variant="ghost" size="sm" className="text-slate-300 hover:bg-white/10 hover:text-white" onClick={() => setDeleteQuoteId(quote.id)} disabled={deleteMutation.isPending}>
