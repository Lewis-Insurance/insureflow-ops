    ...
    <div className="mt-1 space-y-1">
      {linkedNaic ? (
        <p className="text-xs text-muted-foreground">
          NAIC: <span className="cc-num">{linkedNaic}</span> from the carrier directory record{' '}
          {linkedName || 'for this carrier'}
        </p>
      ) : (
        <p className="text-xs text-cc-warning">
          No NAIC on file for {linkedName || 'this carrier'}, so certificates will show the NAIC box
          as missing.{' '}
          <Link
            to={`/carriers?carrier=${carrierInfo.id}`}
            className="font-semibold underline-offset-2 hover:underline"
          >
            Add it on Carriers
          </Link>
