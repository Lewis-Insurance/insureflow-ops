            ...
            <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <p className="max-w-3xl text-sm text-slate-300 md:text-base">One full-width workspace for the renewal, follow-up commitment, quote strategy, document handling, and live servicing context.</p>
                </div>
              ...
              <div className="flex w-full flex-col gap-3 xl:max-w-sm">
                ...
                <Button type="submit" form="ao-renewal-command-form" className="h-12 rounded-2xl bg-lime-300 text-slate-950 hover:bg-lime-200" disabled={updateMutation.isPending}>{updateMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save Renewal</Button>
              </div>
            </div>

            <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <div className={heroTile}><div className="flex items-start justify-between"><div><div className="text-xs uppercase tracking-[0.2em] text-slate-500">Renewal Window</div><div className="mt-3 text-2xl font-semibold text-white">{renewalWindowLabel}</div></div><Clock3 className="h-5 w-5 text-slate-400" /></div><p className="mt-3 text-sm text-slate-400">Renewal date {formData.renewal_date ? formatLocalDateDisplay(formData.renewal_date) : "not set"}</p></div>
              <div className={heroTile}><div className="flex items-start justify-between"><div><div className="text-xs uppercase tracking-[0.2em] text-slate-500">Current Follow-Up</div><div className="mt-3 text-2xl font-semibold text-white">{commandStateLabel}</div></div><CalendarClock className="h-5 w-5 text-slate-400" /></div><p className="mt-3 text-sm text-slate-400">{followUpDraft.date ? formatLocalDateDisplay(followUpDraft.date) : "No date committed"}</p></div>
              <div className={heroTile}><div className="flex items-start justify-between"><div><div className="text-xs uppercase tracking-[0.2em] text-slate-500">Premium</div><div className="mt-3 text-2xl font-semibold text-white">{formatCurrency(formData.current_premium)}</div></div><DollarSign className="h-5 w-5 text-slate-400" /></div><p className="mt-3 text-sm text-slate-400">{formData.term_months ? `${formData.term_months}-month term` : "Term not set"}</p></div>
              <div className={heroTile}><div className="flex items-start justify-between"><div><div className="text-xs uppercase tracking-[0.2em] text-slate-500">Assigned Owner</div><div className="mt-3 text-2xl font-semibold text-white">{assignedProfile?.full_name || "Unassigned"}</div></div><UserRound className="h-5 w-5 text-slate-400" /></div><p className="mt-3 text-sm text-slate-400">Last contact {formData.last_contact_date ? formatLocalDateDisplay(formData.last_contact_date) : "not logged"}</p></div>
            </div>
          </div>
