        ...
        <div className="mx-auto flex w-full max-w-[1800px] flex-col gap-6">
              ...
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm text-slate-400">Use this to judge recency, not as a status replacement.</p>
                  </div>
                </div>
              </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.35fr_0.95fr] xl:items-start">
            <form id="ao-renewal-command-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <Card className={cn(surfaceCard, "rounded-3xl")}><CardHeader className="pb-4"><CardTitle className={sectionTitle}>Key Exceptions</CardTitle><CardDescription className={sectionDescription}>Only the details that actually change the work.</CardDescription></CardHeader><CardContent className="grid gap-4 md:grid-cols-3"><div className={heroTile}><div className="flex items-center gap-2 text-sm font-medium text-white"><AlertCircle className="h-4 w-4 text-amber-300" /> Missing / weak detail</div><p className="mt-3 text-sm leading-6 text-slate-300">{!formData.current_premium ? "Current premium is missing." : !formData.term_months ? "Policy term is missing." : !formData.last_contact_date ? "Last contact is missing." : formData.status === "waiting_on_insured" && !formData.follow_up_date ? "Waiting on insured without a follow-up date." : "No obvious blockers right now."}</p></div><div className={heroTile}><div className="text-xs uppercase tracking-[0.18em] text-slate-500">3-year losses</div><div className="mt-2 text-2xl font-semibold text-white">{formData.losses_3yr || "0"}</div><p className="mt-2 text-sm text-slate-400">Only matters if it changes the retention conversation.</p></div><div className={heroTile}><div className="text-xs uppercase tracking-[0.18em] text-slate-500">Oldest in household</div><div className="mt-2 text-2xl font-semibold text-white">{formData.oldest_in_household || "—"}</div><p className="mt-2 text-sm text-slate-400">Reference detail, not a main action driver.</p></div></CardContent></Card>
            </form>
